...
| Song recognition | Optional secondary 10-second stage sample through ACRCloud plus explicit crowd confirmation; never blocks the map |
...
portrait for all seven stages and the walking corridors between them. It is
generated by the partner CrowdSim agent engine over a reduced OSM path graph,
aggregated into H3 cells with a five-contributor floor, and then loops locally
without a runtime generator. It includes at least three visibly different
states:
