...
2. **setlist.fm gives us what they play.** Recent tour setlists narrow an artist to roughly 20–40 live songs. JamBase does *not* expose setlists; this is a separate source.
...
audio clip ─────────> ACRCloud ───────┐
                                      │
setlist.fm candidates ────────────────┼──> GPT-5 resolver ──> confidence
  (frequency prior, no training)      │          │
                                      │          ├─ below threshold? stay pending
human votes / free text ──────────────┘          └─ above threshold? post to setlist
```
...
└───────┼──────────────┼───────────────┼──────────┘
        │              │               │
  ┌─────▼─────┐ ┌─────▼─────┐ ┌─────▼─────┐ ┌──────▼──────┐
  │ ACRCloud  │ │  JamBase  │ │setlist.fm │ │   OpenAI    │
  │fingerprint│ │ lineup ·  │ │ candidate │ │  resolver   │
  │           │ │ set times │ │   prior   │ │  + agent    │
  └───────────┘ └───────────┘ └───────────┘ └─────────────┘
                        │
                 ┌──────▼──────┐
                 │   Convex    │
...
**Secrets discipline:** ACRCloud, JamBase, and OpenAI keys all live in Sites hosted environment variables and are only ever touched by server routes. Never the client. JamBase v3 auth is a Bearer token in the Authorization header, which would trip CORS from a browser anyway.
...
- **Resolver** — GPT-5 with structured outputs, arbitrating ACRCloud confidence + JamBase candidates + human votes into a single track guess with a confidence score. Strict JSON, no prose.
...
- **Artist resolution + third-party IDs** — matched MusicBrainz MBIDs are what let us key into setlist.fm, and matched Spotify IDs give the catalog fallback tier.
...
- Supports custom fingerprint buckets, so unreleased or DJ-edit material we capture once can be matched on later plays.

### setlist.fm (not a sponsor, but load-bearing)
- Recent setlists per MusicBrainz MBID → the candidate prior
- Free key via `x-api-key`, ~2 req/s and 1,440 req/day, attribution required
- **Non-commercial only.** Owned by Live Nation via Ticketmaster. Commercial use needs a negotiated license
...
MediaRecorder clip capture → server route → ACRCloud. Show the raw result. No consensus yet.

**Phase 3 — the differentiator (60 min)**
JamBase set times decide the active artist per stage and hand us the MusicBrainz MBID. Pull recent setlist.fm setlists for that MBID into `candidates` as a frequency-weighted prior — this is counting, not training. Wire the GPT-5 resolver over ACRCloud + candidates + votes. Threshold gate before posting.
...
| **setlist.fm has thin coverage for an artist** | Fall back to full catalog via matched Spotify/MusicBrainz IDs. Weaker prior, same architecture |
| **setlist.fm is non-commercial-only** | Fine for the prototype. Commercial licensing must be arranged before any paid product ships |
...
Capture a clip live. Show ACRCloud returning something weak. Show the JamBase candidate list narrowing it. Show the resolver picking. Show the vote count crossing threshold and the track posting to the setlist.
